---
description: Humanoid robotics - bipedal locomotion, whole-body control, and embodied AI from Boston Dynamics, Tesla, Unitree, and Toyota.
icon: person
---

# Humanoids

Humanoid robots are the frontier of embodied AI - from Tesla Optimus to Boston Dynamics Atlas, whole-body dynamics and bipedal control define this domain.

### In this section

| Topic | Link |
|-------|------|
| Bipedal Locomotion | [bipedal-locomotion.md](#) |
| Whole-Body Control | [whole-body-control.md](#) |
| Dexterous Manipulation | [dexterous-manipulation.md](#) |

